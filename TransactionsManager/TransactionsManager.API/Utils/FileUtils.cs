...
namespace TransactionsManager.API.Utils
{
    ...
    {
        public static async Task<FileContentResult> GetFileContent(string path, string fileName, HttpContext context, string defaultContentType = null)
        {
            string contentType = defaultContentType ?? GetMimeTypeByWindowsRegistry(Path.GetFileName(path));
